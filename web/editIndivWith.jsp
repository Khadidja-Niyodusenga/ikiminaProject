...
<head>
    ...
    <title>Edit Withdraw Request</title>
    <style>
         body, html {
    ...
    <div class="container">
        <aside class="sidebar">
            ...
            <nav>
                ...
                <a href="individualContribution.jsp"><img src="guestnew.jfif" alt="Guest">Contribution</a>
                <a href="viewIndividualWith.jsp"><img src="normal.jfif" alt="Normal Staff"> Withdraw</a>
                <a href="individualLoan.jsp"><img src="credit comi.png" alt="Credit Committee">loan</a>
...
<!--                <a href="vicepresidentdashboard.html"><img src="vice.png" alt="Vice President">Vice President</a>
    ...
    <%
        ...
        try {
            ...
            // Fetch the existing withdraw request details
            String sql = "SELECT mid, request_date, amount_requested, reason_to_withdraw, withdraw_method FROM withdraw WHERE rwid = ?";
    ...
    %>
    <form action="UpdateWithdrawIndiv" method="post">
        <input type="hidden" name="rwid" value="<%= rwid %>">
       
        <input type="hidden" id="mid" name="mid" value="<%= mid %>" required>
        <label for="request_date">Request Date:</label>
        <input type="date" id="request_date" name="request_date" value="<%= requestDate.toLocalDate() %>" required>
        <label for="amount_requested">Amount Requested:</label>
        <input type="number" id="amount_requested" name="amount_requested" step="0.01" value="<%= amountRequested %>" required>
        <label for="reason_to_withdraw">Reason to Withdraw:</label>
        <textarea id="reason_to_withdraw" name="reason_to_withdraw" rows="4" required><%= reasonToWithdraw %></textarea>
         <label for="withdraw_method">Withdraw Method:</label>
        <input type="text" id="withdraw_method" name="withdraw_method" value="<%= withdrawMethod %>" required>
        <button type="submit">Update</button>
    </form>
    <%
            } else {
    %>
    <p>Withdraw request not found.</p>
    <%
            }
        } catch (Exception e) {
            e.printStackTrace();
